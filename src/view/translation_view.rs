...
use crate::state::server_state::ServerAction;
use crate::state::translation_model::TransAction;
...
pub fn translation_menu_bar(state: &Translator) -> Element<'_, Message> {
    row![
...
fn check_mark<'a>() -> Container<'a, Message> {
...
pub fn server_menu(state: &Translator) -> Item<'_, Message, Theme, Renderer> {
    let url = &state.server_state.url;
    Item::with_menu(
        menu_button("server"),
        Menu::new(vec![
            Item::new(url_input(state)),
            Item::new(
                container(button(text("connect")).on_press(
                    ServerAction::Connect(url.trim().is_empty().not().then(|| url.clone())).into(),
                ))
...
pub fn url_input(state: &Translator) -> Container<'_, Message> {
    container(
        row![
            container(text("url:").center()),
